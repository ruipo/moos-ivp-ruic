...
bool CFrontEstimate::OnStartUp()
{
  ...
  string sVal;
    ...
    if(m_MissionReader.GetConfigurationParam("reporting_vname",sVal))
    {
      m_reporting_vname = sVal;
    }  
...
bool CFrontEstimate::OnConnectToServer()
{
  ...
  Register("COMMS_REC",0);
  Register("COLLABED",0);
  Register("UCTD_MSMNT_REPORT",0);
  Register("UCTD_MSMNT_REPORT_COMM",0);
...
bool CFrontEstimate::Iterate()
{
  ...
  if (completed && !comms_rec){
    unsigned int i, vsize = ctd_list.size();
    for(i=0; i<vsize; i++) {
      //string comm_vec = "src_node="+vname+",dest_node=all,var_name=UCTD_MSMNT_REPORT_COMM,string_val="+"\""+ctd_list[i]+"\"";
      Notify("NODE_MESSAGE_LOCAL",ctd_list[i]);
    }
    string comm_end = "src_node="+vname+",dest_node=all,var_name=COLLABED,string_val=true";
    Notify("NODE_MESSAGE_LOCAL",comm_end);
    // string comm_end1 = "src_node="+vname+",dest_node=all,var_name=SURVEY_UNDERWAY,string_val=false";
    // Notify("NODE_MESSAGE_LOCAL",comm_end1);
  }
...
// This loop checks ALL messages
  for(p=NewMail.begin(); p!=NewMail.end(); p++)
    ...
    string comm_msg = "src_node="+vname+",dest_node=all,var_name=UCTD_MSMNT_REPORT_COMM,string_val="+"\""+value+"\"";
    ctd_list.push_back(comm_msg);
    //Notify("NODE_MESSAGE_LOCAL",comm_msg);
	}
      else if (rMsg.m_sKey == "UCTD_MSMNT_REPORT_COMM" && in_survey)
  {
    value = rMsg.m_sVal;  
    CMeasurement buf;
    buf = anneal.parseMeas(value);
